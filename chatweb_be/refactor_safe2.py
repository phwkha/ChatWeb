...
def process_file(filepath):
    ...
    # Find Translator.tolocale("...") with or without following arguments
    for i, line in enumerate(new_lines):
        ...
        # also find {valid.*}
        matches2 = re.finditer(r'"(\{valid\.[^"\\]+\})"', line)
    ...
    for i, line in enumerate(new_lines):
        for val, name in constants.items():
            if val.startswith("{valid"):
                ...
                pattern = r'Translator\.tolocale\("' + re.escape(val) + r'"'
                repl = f'Translator.tolocale({name}'
                new_lines[i] = re.sub(pattern, repl, new_lines[i])
